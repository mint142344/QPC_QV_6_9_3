...
/* 宏: 在 QV_onIdle() 中让 CPU 进入低功耗睡眠模式 */
#define QV_CPU_SLEEP()   \
    do {                 \
        __wfi();         \
        QF_INT_ENABLE(); \
...
/* 初始化 Cortex-M3/M4/M4F 上的 QV 内核 */
#define QV_INIT()               QV_init()
void QV_init(void);

/* 宏: 处理 ARM Erratum 838869 的推荐方法
 * 说明: 对于 Cortex-M3/M4/M7, 需要在 ISR 结束前执行 DSB（数据同步屏障）指令,
 * 以确保所有内存操作都已完成.
 * 该宏应插入在所有中断服务程序(ISR)的末尾。
 */
#define QV_ARM_ERRATUM_838869() __asm("dsb")

#endif

#include "qv.h" /* QV 平台无关公共接口 */
